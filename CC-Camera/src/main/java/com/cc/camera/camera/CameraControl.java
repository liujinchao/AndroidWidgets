...
public class CameraControl implements ICameraControl {
    ...
    @Override
    public void setDisplayOrientation(@CcCameraView.Orientation int displayOrientation) {
        this.displayOrientation = displayOrientation;
        switch (displayOrientation) {
            case CcCameraView.ORIENTATION_PORTRAIT:
                rotation = 90;
                break;
            case CcCameraView.ORIENTATION_HORIZONTAL:
                rotation = 0;
                break;
            case CcCameraView.ORIENTATION_INVERT:
                rotation = 180;
                break;
            default:
                rotation = 0;
                        ...
                        try {
                            camera.autoFocus(new Camera.AutoFocusCallback() {
                                ...
                                public void onAutoFocus(boolean success, Camera camera) {
                                    System.out.print("聚焦成功："+success);
                                }
                            });
                        } catch (Throwable e) {
                            // startPreview是异步实现，可能在某些机器上前几次调用会autofocus failß
                            System.out.print("聚焦失败："+e.getLocalizedMessage());
