...
import com.cc.camera.camera.CameraControl;
...
import com.cc.camera.view.utils.DimensionUtil;
import com.cc.camera.view.utils.ImageUtil;
 ...
 */
public class CcCameraView extends FrameLayout {
    ...
    public void setOrientation(@Orientation int orientation) {
        getCameraControl().setDisplayOrientation(orientation);
    }

    public void setPermissionCallback(PermissionCallback callback){
        getCameraControl().setPermissionCallback(callback);
    }

    public void refreshPermission(){
        getCameraControl().refreshPermission();
    }
    ...
    public CcCameraView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CcCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setMaskType(@CcMaskView.MaskType int maskType, final Context ctx) {
...
//        int hintResourceId = 1;
        ...
        boolean isNeedSetImage = false;
        switch (maskType) {
            case CcMaskView.MASK_TYPE_NONE:
            default:
...
//            hintView.setImageResource(hintResourceId);
        ...
        if (maskType == CcMaskView.MASK_TYPE_NONE && isEnableScan) {
...
//        }
    ...
    private void init() {
        ...
        maskView = new CcMaskView(getContext());
        maskView.setMaskType(CcMaskView.MASK_TYPE_NONE);
        addView(maskView);
